...
import { chargesActions, getCharges } from '@features/charges/chargesSlice';
import useChargesPagination from '@features/charges/hooks/useChargesPagination';
import { CHARGES_LIMIT } from '@features/charges/constants';
import server from '../../../mocks/api/server';
import { Wrapper } from '../../../wrapper';
import { rootStateMock } from '../../../mocks/stateMock';

const useDispatchMock = jest.spyOn(ReactRedux, 'useDispatch');
const changeBoundIdsActionMock = jest.spyOn(chargesActions, 'changeBoundIds');
...
describe('[HOOK]: useChargesPagination', () => {
  ...
  test('isNextDisabled and isPrevDisabled flags should be true.', () => {
    const { result } = renderHook(() => useChargesPagination(), { wrapper: Wrapper });
  ...
  test('should fetch next chunk of items, isNextDisabled should be false.', () => {
    ...
    const state = {
      ...rootStateMock,
      charges: {
        ...rootStateMock.charges,
        startingAfter,
        items: [],
      },
    };

    const { result } = renderHook(() => useChargesPagination(), {
    ...
    expect(getCharges).toBeCalledWith({ startingAfter, endingBefore: undefined, limit: undefined });
  });
  ...
  test('should fetch previous chunk of items, isPrevDisabled should be false.', () => {
    ...
    expect(getCharges).toBeCalledWith({ endingBefore, startingAfter: undefined, limit: undefined });
  });
  // eslint-disable-next-line max-len
  test('in case chunk with current bounds already fetched, should return next chunk from cache by startingAfter ID.', () => {
    const startingAfter = 5;
    const endingBefore = 1;
    const items = Array.from({ length: CHARGES_LIMIT * 2 }).map((_, idx) => ({ id: idx + 1 }));
  ...
  test('in case chunk with current bounds already fetched, should return previous chunk from cache by endingBefore ID.', () => {
    const items = Array.from({ length: CHARGES_LIMIT * 3 }).map((_, idx) => ({ id: idx + 1 }));
    const startingAfter = items[items.length - 1].id;
    const endingBefore = items[items.length - CHARGES_LIMIT].id;
    const endingBeforeIdx = items.findIndex(({ id }) => id === endingBefore);
    const startingAfterIdx = items.findIndex(({ id }) => id === startingAfter);
    const nextStartingAfter = items[startingAfterIdx - CHARGES_LIMIT].id;
    const nextEndingBefore = items[endingBeforeIdx - CHARGES_LIMIT].id;
    const state = {
      ...rootStateMock,
      charges: {
        ...rootStateMock.charges,
